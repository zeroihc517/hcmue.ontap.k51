...
function generateAndExportRandomExamPDF() {
    ...
    // Cảnh báo nhẹ nếu xuất quá nhiều đề
    if (numberOfExams > 30) {
    ...
    <head>
        ...
        <script>
            window.MathJax = {
                ...
                startup: { typeset: false } // TẮT TỰ ĐỘNG TYPESET TOÀN TRANG ĐỂ TRÁNH GIẬT/ĐỨNG MÁY
            };
        </script>
    ...
    for (let examIndex = 0; examIndex < numberOfExams; examIndex++) {
        ...
        if (p1.length > 0) {
            ...
            p1.forEach((q) => {
                ...
                // HÀM LỌC BỎ CÁC MÃ LỆNH MATHJAX
                ...
                // Tinh chỉnh ngưỡng ký tự: > 45 mới tụt xuống 4 hàng; > 18 chia thành 2 hàng x 2 cột
                if (maxLen > 45) {
                    optionsTableHtml = `
    ...
    // 5. TRANG BẢNG ĐÁP ÁN DÀNH CHO GIÁO VIÊN
    html += `
    ...
    // kỊCH BẢN XỬ LÝ TOÁN VÀ MỞ CỬA SỔ IN BẤT ĐỒNG BỘ NẠP THEO LUỒNG ĐỂ KHÔNG BỊ TREO
    html += `
    </div>
    <script>
